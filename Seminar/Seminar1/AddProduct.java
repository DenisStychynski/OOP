...
public class AddProduct {  // - Interface segregation principle (Принцип разделения интерфейса)
    //В интерфейсе есть специализированные интерфейсы
        ...
        while (true) {                                                
            ...
            for (i=0; i<list_product.size(); i++) {                                                 // пробегает весь список продуктов и ищет есть ли такой в базе
                if(list_product.get(i).getID()==id_product){                                        // Если находит такой товар
                    ...
                    System.out.print("Введите кол-во добавляемого товара: ");                     // Запрашивает какое кол-во товара добавляется в автомат
            ...
            if(i == list_product.size()){                                     // если прошли весь список, но товара не нашли, то создаем новую запись
                ...
                if (id >0){
                    ...
                    if(idCommand > 2){                                             // от выбранной команды ранее добавляем горячий напиток
                        ...
                        scan.nextLine();
                        if(cat == 1){
                            list_hot.add(new HotBeverage(id,name, price, quantity, new GregorianCalendar(YEAR, MONTH, DAY),volume,temperature));
                        }      
                    }
                }
            }
